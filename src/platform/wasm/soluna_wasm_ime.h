...
extern void soluna_wasm_dom_set_color(int has_color, uint32_t color);
...
static void
soluna_wasm_call_set_color(int has_color, uint32_t color) {
#if defined(__EMSCRIPTEN_PTHREADS__)
    if (!emscripten_is_main_browser_thread()) {
        emscripten_async_run_in_main_runtime_thread(EM_FUNC_SIG_VII, soluna_wasm_dom_set_color, has_color, color);
        return;
    }
#endif
    soluna_wasm_dom_set_color(has_color, color);
}
...
soluna_wasm_apply_rect(void) {
    ...
    soluna_wasm_call_setup();
    soluna_wasm_call_set_color(g_soluna_ime_rect.has_text_color ? 1 : 0, g_soluna_ime_rect.text_color);
